...
violinPlot <- function(icombined_tpm, main_title, plot_mode = "static") {
  ...
  p1 <- plotly::plot_ly(icombined_tpm, type = 'violin') %>%
    add_trace(y = ~log2(tpm_bcbio+0.5), box = list(visible = T),  meanline = list(visible = F),
                       x = 'Sample_bcbio', legendgroup = 'run1', text = icombined_tpm$target_id, name = 'bcbio', color = 'orange') %>%
    add_trace(y = ~log2(tpm_dragen+0.5), box = list(visible = T),  meanline = list(visible = F),
                       x = 'Sample_dragen', legendgroup = 'run2', text = icombined_tpm$target_id, name = 'dragen', color = 'green') %>%
  ...
  p2 <- plotly::plot_ly(icombined_tpm, type = 'violin') %>%
    add_trace(y = c(~log(quantile(tpm_bcbio)[1]+0.5), ~log(quantile(tpm_bcbio)[3]+0.5)+0.5), box = list(visible = T),  meanline = list(visible = F),
                       x = 'Sample_bcbio', legendgroup = 'run1', text = icombined_tpm$target_id, name = 'bcbio', color = 'orange') %>%
    add_trace(y = c(~log(quantile(tpm_dragen)[1]+0.5), ~log(quantile(tpm_dragen)[3]+0.5)+0.5), box = list(visible = T),  meanline = list(visible = F),
              x = 'Sample_dragen', legendgroup = 'run2', text = icombined_tpm$target_id, name = 'dragen', color = 'green') %>%
...
datatable(bcbio.dragen.gene.counts.filtered, rownames = FALSE, filter="top", options = list(pageLength = 5, scrollX=T))
...
```{r tpm-plots, message=FALSE, warning=FALSE}
