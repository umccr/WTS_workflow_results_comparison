...
diffFiles <- function(file_names) {
  ...
  if (stringr::str_detect(file_names[1], pattern = 'counts$')) {
    ...
    input1 <- read.table(file_names[1], sep="\t", as.is=TRUE, header=TRUE) %>%
      mutate(round(est_counts, digits = 2), round(tpm, digits=2))
    input2 <- read.table(file_names[2], sep="\t", as.is=TRUE, header=TRUE) %>%
      mutate(round(est_counts, digits = 2), round(tpm, digits=2))
    join_by <-  c('target_id', 'est_counts', 'tpm')
  }
...
datatable(lines_count, rownames = FALSE, filter="top", options = list(pageLength = 5, scrollX=T))
...
datatable(lines_abundance, rownames = FALSE, filter="top", options = list(pageLength = 5, scrollX=T))
...
```{r fusions, message=FALSE, warning=FALSE}
...
datatable(lines_fusion, rownames = FALSE, filter="top", options = list(pageLength = 5, scrollX=T))
#function call for finding the differences between fusion calls from both runs
