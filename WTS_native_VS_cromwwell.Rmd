...
scatterPlot <- function(icombined_tpm, main_title, plot_mode = "static") {
  plotly::plot_ly(icombined_tpm, x = ~log(tpm_run1), y = ~log(tpm_run2), type = 'scattergl', 
...
violinPlot <- function(icombined_tpm, main_title, plot_mode = "static") {
  ...
  plotly::plot_ly(icombined_tpm, type = 'violin') %>%
    add_trace(y = ~log(tpm_run1), box = list(visible = T),  meanline = list(visible = F),
                       x = 'Sample_bcbio_native', legendgroup = 'run1', text = icombined_tpm$target_id, name = 'bcbio_native') %>%
    add_trace(y = ~log(tpm_run2), box = list(visible = T),  meanline = list(visible = F),
                       x = 'Sample_bcbio_cromwell', legendgroup = 'run2', text = icombined_tpm$target_id, name = 'bcbio_cromwell') %>%
