...
salmon <- list.files("/Users/kanwals/Documents/UMCCR/data/WTS_comparison/CCR180072_VPT_WH16/bcbio_native/run_1/salmon/", pattern = "quant.sf", full.names = T)
featurecounts <- read.table("/Users/kanwals/Documents/UMCCR/data/WTS_comparison/CCR180072_VPT_WH16/bcbio_native/run_1/CCR180072_VPT_WH16-ready.counts", sep="\t", as.is=TRUE) 
...
feature.salmon.counts <- inner_join(featurecounts, salmon.counts, by = "gene") %>%
  dplyr::mutate(percentage_diff = ifelse(count.x > count.y, (count.x - count.y)/count.x *100, (count.y - count.x)/count.y *100)) %>%
  dplyr::mutate_if(is.numeric, round,2)
colnames(feature.salmon.counts) <- c("gene", "featurecounts", "salmoncounts", "percentage_diff") 
...
gene_info = gene_info[!duplicated(gene_info$gene) | !duplicated(gene_info$geneName),]


##### Get genes genomic coordiantes
gene_info <- ensembldb::select(edb, keys=keys, columns=c("GENEID", "GENEBIOTYPE", "GENENAME", "SEQNAME", "GENESEQSTART", "GENESEQEND"), keytype="GENEID")#counts correlation between featurecounts and salmon
ggplot(feature.salmon.counts, 
       aes(x=log2(feature.salmon.counts$featurecounts+1), y=log2(feature.salmon.counts$salmoncounts+1))) +
         geom_point() +
         geom_smooth(method = "lm") +
         geom_abline(slope=1, intercept = 0, color="red") +
         annotate("text", x=15, y=20, label= "spearman cor = 0.89") +
