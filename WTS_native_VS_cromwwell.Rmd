...
###generate scatter plot for tpm values across both runs
comparePlot <- function(file_names, main_title, plot_mode = "static") {
  ...
  #p <- plotly::plot_ly(icombined_tpm, x = ~tpm_run1, y = ~tpm_run2, type = 'scattergl', text = icombined_tpm$target_id, width = 00, height = 400) %>%
  ...
  #  plotly::layout(title = main_title)
    ...
    p <- ggplot(data = icombined_tpm, aes(x = log(tpm_run1), y = log(tpm_run2))) + 
      geom_point(shape = 16, size = 2, show.legend = FALSE, color="firebrick")
    p <- ggplotly(p) %>% toWebGL()
    
    p_log <- ggplot(data = icombined_tpm, aes(x = tpm_run1, y = tpm_run2)) + 
      geom_point(shape = 16, size = 2, show.legend = FALSE, color="firebrick")
    p_log <- ggplotly(p_log) %>% toWebGL()
    
    do.call(grid.arrange, c(p, p_log, list(ncol = 2)))
return( p )
}
